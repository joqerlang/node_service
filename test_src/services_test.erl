...
available()->
    ...
    ?assertEqual([{"dns_service",node_dir_test@asus}],services:available(?CATALOG)),
    ok=application:start(lib_service),
    ok=application:start(log_service),
    ?assertEqual([{"log_service",node_dir_test@asus},
		  {"lib_service",node_dir_test@asus},
		  {"dns_service",node_dir_test@asus}],services:available(?CATALOG)),
...
missing()->
    ?assertEqual([{"dns_service",glurk@asus},
		  {"lib_service",node_dir_test@asus}],services:missing(?CATALOG,?APP_SPEC1)),
    ok=application:start(lib_service),
    ?assertEqual([{"dns_service",glurk@asus}],services:missing(?CATALOG,?APP_SPEC1)),
    ok.

obsolite()->
    ?assertEqual([{"lib_service",node_dir_test@asus}],services:obsolite(?CATALOG,?APP_SPEC2)),
    ok=application:stop(lib_service),
    ?assertEqual([],services:obsolite(?CATALOG,?APP_SPEC2)),
    ok.
